...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get WordPress configuration (French)
    const wpUrl = await settings.get('wordpressFrenchUrl');
    const wpUsername = await settings.get('wordpressFrenchUsername');
    const wpPassword = await settings.get('wordpressFrenchPassword');

    if (!wpUrl || !wpUsername || !wpPassword) {
      return NextResponse.json(
        { error: 'WordPress French credentials not configured' },
        { status: 500 }
      );
    }
